codeunit 50153 E_Invoice_TransferShipments
{
    ...
    procedure WriteSellerDtls(TransferShipHeader: Record "Transfer Shipment Header"; JsonWriter: DotNet JsonTextWriter;
     JObject: DotNet JObject; gl_BilltoEm: text; gl_BilltoPh: Code[12])
    var
        Loc: text;
        Pin: Integer;
        Stcd: Code[15];
    ...
    begin
        ...
        IF glResponse.StatusCode = 200 THEN BEGIN
            ...
            ParseResponse_IRN_DECRYPT(decryptedIRNResponse, ISIRNCancel, TransferShipHeader, IsEwayBill, ISEWayCancel, false);
    ...
    procedure Call_Ewaybill_API(recAuthData: Record "GST E-Invoice(Auth Data)"; JsonString: text; TransferShipHeader: record "Transfer Shipment Header"; IsEwayBill: Boolean; ISEWayCancel: Boolean; WithoutIRN: Boolean)
    var
        genledSetup: Record "General Ledger Setup";
        glHTTPRequest: DotNet HttpWebRequest;
        gluriObj: DotNet Uri;
    ...
    begin
        ...
        servicepointmanager.SecurityProtocol := securityprotocol.Tls12;
        if IsEwayBill then
            gluriObj := gluriObj.Uri(genledSetup."EWAYBILL w/o IRN")
        else
            if ISEWayCancel then
                gluriObj := gluriObj.Uri(genledSetup."Cancel E-Way Bill");
        ...
        IF glResponse.StatusCode = 200 THEN BEGIN
            ...
            signedData := ParseResponse_IRN_ENCRYPT(txtResponse, false, TransferShipHeader, IsEwayBill, ISEWayCancel);
            ...
            Outstr.WRITETEXT(decryptedIRNResponse);*/
            IntraState := true;
            ParseResponse_IRN_DECRYPT(decryptedIRNResponse, false, TransferShipHeader, IsEwayBill, ISEWayCancel, IntraState);
    ...
    procedure ParseResponse_IRN_DECRYPT(TextResponse: text; ISIRNCancel: Boolean; TransferShipHeader: Record "Transfer Shipment Header"; IsEwayBill: Boolean; IsEwayCancel: Boolean; IntraState: Boolean): Text;
    var
        message1: Text;
        CurrentObject: Text;
        FormatChar: label '{}';
        ...
        txtAlert: text;
        CU_EWB_Transfer: Codeunit E_WayBill_Transfer;
    begin
        //Get value from Json Response >>
        ...
        // CurrentObject := COPYSTR(TextResponse,STRPOS(TextResponse,'{')+1,STRPOS(TextResponse,':'));
        // TextResponse := COPYSTR(TextResponse,STRLEN(CurrentObject)+1);
        if TextResponse.Contains('"alert":",') then
            TextResponse := TextResponse.Replace('"alert":",', '"alert":"');
        if TextResponse.Contains(', "') then
        ...
        WHILE p < l DO BEGIN
            ...
            IF STRPOS(ValuePair, ':') > 0 THEN BEGIN
                ...
                CASE CurrentElement OF
                    ...
                    'alert':
                        begin
                            txtAlert := CurrentValue;
                        end;


                END;
                if IsEwayBill and (txtEWBNum = '') then
                    txtEWBNum := txtCancelEwayNum;
            END;
            x := x + 1;
        END;
        ...
        IF recTransferShipHeader.find('-') THEN begin
            if IsEwayBill then
                CU_EWayBill.UpdateHeaderIRN(txtEWBDt, txtEWBNum, txtEWBValid, TransferShipHeader, IntraState)//230622
            else
                if ISIRNCancel then
